<%-- 
    Document   : homepage
    Created on : Sep 18, 2024, 11:47:23 AM
    Author     : Acer
--%>
...
<html lang="en">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      ...
      <script>
           ...
           $(document).ready(function (){
               //alert("hello");
               $.get("http://localhost:8081/availablebooks",function (data,status){
                  //alert(data.length); 
                  
                  $("#tabl tbody").empty(); 
				     data.forEach(function (item) { 
				    	 $("#tabl tbody").append('<tr> <td> '+item.id+' </td> <td> '+item.name+' </td>  <td> '+item.author+' </td><td> '+item.price+' </td><td><button type="button" id="btn" class="addmybook">Add To Mybooks</button></td><td><a href="editbook.jsp"><button type="button" id="btn" class="getvalues">Edit</button></a></td><td><button type="button" id="btn" class="getid">Delete</button></td> </tr>'); 	    	
        });  
               });
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json",
        url: "http://localhost:8081/addmybook", 
        data: JSON.stringify({ 
            id: id, 
            name: name, 
            author: author, 
        ...
        success: function(data) {
            if (data === "success") {
                alert("Book added to My Books Successfully!");
               ...
               $(document).on('click', '#tabl .addmybook', function() {
                ...
                $(document).on('click', '#tabl .getvalues', function() {
                   ...
                   console.log("Edit button clicked!");
                    ...
                    var id = row.find('td:eq(0)').text();
                    console.log("ID: " + id);
                  $.get("http://localhost:8081/editbook/"+id,function(data,status){
                      console.log(data);
                      $("#id").val(data.id);
